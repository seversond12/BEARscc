...
randomizer<-function(x,parameters,total_sampling){
    ...
    if (p<0) {p=0}
    if (p>1) {p=1}
    hx1 <- rnbinom(total_sampling*p,size = x^2/
        ((2^(Mean2SDcor_slope*log2(x)+Mean2SDcor_intercept+
        Mean2SDcor_residual*sd_inflate))^2-x),mu = x)
    hx2 <- rpois(total_sampling*(1-p),x)
    sample(append(hx1, hx2), size=1, replace=TRUE)
}

##Count permuter changes zeros and calls the randomizer function.
permute_count<-function(x, probs4detection.k, probabilityA, 
    ...
    if (x==0) {
        ...
        if (nx>0) {
            nx<-randomizer(probs4detection.k[as.character(nx),]$Counts, 
                parameters, total_sampling)
        }
    }
    else {
...
genewise_permute_count<-function(x, probs4detection.k, probs4detection.genes, 
    parameters, total_sampling){
    probabilityA<-probs4detection.genes[gsub("-",".",x[1]),]
    force(total_sampling)
    apply(data.frame(as.numeric(x[-1])),1, `permute_count`, probs4detection.k, 
        probabilityA=probabilityA, parameters, total_sampling)
}
...
execute_noiseinjected_counts<-function(n, noise_parameters,total_sampling){
    ...
    probs4detection.genes<-t(data.frame(noise_parameters$bayes_parameters, 
        row.names = "k")[,4:eval(dim(noise_parameters$bayes_parameters)[2]-1)])
...
create_noiseinjected_counts<-function(noise_parameters, 
    total_sampling=2500, n=3){
    force(total_sampling)
    ...
    noisy_counts.list<-lapply(vector, `execute_noiseinjected_counts`, 
        noise_parameters=noise_parameters, total_sampling=total_sampling)
    noisy_counts.list$Original_counts<-
        data.table(noise_parameters$original.counts, keep.rownames = TRUE)
