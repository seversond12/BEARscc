...
create_cm<-function(cluster_labels, names){
  ...
  sapply(cluster_labels, function(x){sapply(cluster_labels, function(y){as.numeric(x==y)})})
}
...
calculate_cluster_metrics_by_cluster<-function(clusters,cluster_labels,consensus_matrix){
  in.cluster<-as.character(names(cluster_labels[cluster_labels==clusters]))
  out.cluster<-as.character(names(cluster_labels[!(cluster_labels==clusters)]))
  if (length(in.cluster)>1){
    inside.stability<-(sum(consensus_matrix[in.cluster,in.cluster, drop=FALSE])-length(in.cluster))/(length(in.cluster)^2-length(in.cluster))
  } else {
    inside.stability<-1
  }
  promiscuity.matrix<-as.matrix(consensus_matrix[in.cluster,out.cluster, drop=FALSE])
...
calculate_cluster_metrics<-function(cluster_labels,consensus_matrix,cluster_names){
  ...
  metric_list<-lapply(clusters, `calculate_cluster_metrics_by_cluster`, consensus_matrix=consensus_matrix, cluster_labels=cluster_labels)
  metric_list<-melt(metric_list, id.vars=c("size"))
  colnames(metric_list)[c(2,4)]<-c("metric","rn")
  metric_list
}

calculate_cell_metrics_by_cluster<-function(cluster_names, cluster_labels, consensus_matrix){
  in.cluster<-as.character(names(cluster_labels[cluster_labels==cluster_names]))
  out.cluster<-as.character(names(cluster_labels[!(cluster_labels==cluster_names)]))
  if (length(in.cluster) < length(out.cluster)){
    if (length(in.cluster)==1){
      inside.stability<-1
      outside.promiscuity<-max(consensus_matrix[in.cluster, out.cluster])
      score<-1-outside.promiscuity
      cell_metric.dt<-data.frame(cell=as.character(in.cluster[1]),score=score, Promiscuity=outside.promiscuity, Stability=inside.stability, cluster.size=length(in.cluster))
      cell_metric.dt$cluster.size<-length(in.cluster)
    } else {
      stability.dt<-data.table(data.frame(consensus_matrix[in.cluster,in.cluster, drop=FALSE]), keep.rownames = TRUE)
      stability.dt<-stability.dt[,.(Stability = (rowSums(.SD)-1)/(length(in.cluster)-1)), keyby=rn]
      promiscuity.dt<-data.table(data.frame(consensus_matrix[in.cluster,out.cluster, drop=FALSE]), keep.rownames = TRUE)
      ...
      cell_metric.dt$score<-cell_metric.dt$Stability-cell_metric.dt$Promiscuity
      cell_metric.dt$cluster.size<-length(in.cluster)
    }
    ...
    if (length(in.cluster)==dim(consensus_matrix)[1]) {
      ...
      cell_metric.dt$score<-cell_metric.dt$Stability-cell_metric.dt$Promiscuity
      cell_metric.dt$cluster.size<-length(in.cluster)
      colnames(cell_metric.dt)<-c("cell", "Stability", "Promiscuity", "score", "cluster.size")
    }
  }
  cell_metric.dt
}
...
calculate_cell_metrics<-function(cluster_labels,consensus_matrix,clusters){
  ...
  cell_metric_list<-data.table(melt(lapply(cluster_names, `calculate_cell_metrics_by_cluster`, consensus_matrix=consensus_matrix, cluster_labels=cluster_labels), id.vars=c("cell", "cluster.size")))
...
compute_consensus<-function(cluster_labels, plot=FALSE, file="Rplot"){
...
report_cell_metrics<-function(cluster_labels,consensus_matrix){
  ...
  cell.scores<-list()
  cell.scores<-lapply(cluster_labels, `calculate_cell_metrics`,consensus_matrix=consensus_matrix, clusters=clusters)
  ...
  colnames(cell.scores)<-c("Cluster identity", "Cell", "Cluster size", "Metric", "Value", "Clustering")
  cell.scores
}
...
report_cluster_metrics<-function(cluster_labels,consensus_matrix, weighted_mean=FALSE,plot=FALSE, file="Rplot"){
  ...
  cluster.scores$X1<-melt(data.table(rn="1",score=oneclust.stability,Promiscuity=0,Stability=oneclust.stability, size=dim(consensus_matrix)[1]), id.vars=c("rn", "size"))
  colnames(cluster.scores$X1)[3]<-"metric"
  cluster.scores<-append(cluster.scores,lapply(cluster_labels, `calculate_cluster_metrics`,consensus_matrix=consensus_matrix, cluster_names=cluster_names))
  ...
  colnames(cluster.scores.m)<-c("Cluster identity", "Cluster size", "Metric", "Value", "Clustering", "Singlet", "Clustering Mean")
  cluster.scores.m
}
