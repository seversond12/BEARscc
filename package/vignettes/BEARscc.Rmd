...
and spike-in counts measurement `data.frame` objects. Although 
`estimate_noiseparameters()` speed is invariant to data size, it does take 
a couple of minutes to estimate noise with a reasonably granular 
`alpha_granularity` parameter, so please be patient when running the 
following code.  
...
desired number of simulated technical replicates. The resulting object is
a list, where each element is a simulated technical replicate, and one 
element is the original counts matrix. 
