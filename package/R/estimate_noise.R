...
prepare_data<-function(x,y, estimateSizeFactors, normseq){
    if (normseq==TRUE){
    sizenorms<-estimateSizeFactorsForMatrix(x)
    x<-sweep(x,2,sizenorms,"/")
    }
    x$transcripts<-y[rownames(x),1]
    x
}

#as above for counts table
prepare_data_counts<-function(x,z,estimateSizeFactors){
  sizenorms<-estimateSizeFactorsForMatrix(x)
  z<-sweep(z,2,sizenorms,"/")
  z
}
...
calculate_cluster_stability<-function(y,x,z){
  names(y)<-z
  error.df<-data.frame(rn="", score=-1,Promiscuity=-1,Stability=-1, size=-1)
  for (i in unique(y)){
    in.cluster<-as.character(names(y[y==i]))
    out.cluster<-as.character(names(y[!(y==i)]))
    if (length(in.cluster)>1){
      inside.stability<-(sum(x[in.cluster,in.cluster])-length(in.cluster))/(length(in.cluster)^2-length(in.cluster))
    } else {
      inside.stability<-1
    }
    promiscuity.matrix<-as.matrix(x[in.cluster,out.cluster])
    symmetry<-1-(length(in.cluster)/(dim(x)[1]-1))
    qt<-quantile(promiscuity.matrix,symmetry)
    promiscuity.matrix[promiscuity.matrix<qt]<-NA
    outside.promiscuity<-mean(promiscuity.matrix, na.rm = TRUE)
    score<-inside.stability-outside.promiscuity
    error.df1<-data.frame(rn=as.character(i),score=score, Promiscuity=outside.promiscuity, Stability=inside.stability, size=length(in.cluster))
    error.df2<-rbind(error.df, error.df1)
    error.df<-error.df2
  }
  error.df[-1,]
}
...
calculate_cell_stability<-function(y,x,z){
  names(y)<-z
  error.df<-data.frame(rn="",cell="", score=-1,Promiscuity=-1,Stability=-1, cluster.size=-1)
  for (i in unique(y)){
    in.cluster<-as.character(names(y[y==i]))
    out.cluster<-as.character(names(y[!(y==i)]))
    for (j in unique(in.cluster)){
      if (length(in.cluster)==1){
        inside.stability<-1
        outside.promiscuity<-max(x[j, out.cluster])
        score<-1-outside.promiscuity
      } else if (length(in.cluster)==(length(in.cluster)+length(out.cluster))) {
        inside.stability<-(sum(x[j,in.cluster])-1)/(length(in.cluster)-1)
        outside.promiscuity<-0
        score<-inside.stability
      } else {
        inside.stability<-(sum(x[j,in.cluster])-1)/(length(in.cluster)-1)
        promiscuity.matrix<-as.matrix(x[j,out.cluster])
        symmetry<-1-(length(in.cluster)/(dim(x)[1]-1))
        qt<-quantile(promiscuity.matrix,symmetry)
        promiscuity.matrix[promiscuity.matrix<qt]<-NA
        outside.promiscuity<-mean(promiscuity.matrix, na.rm = TRUE)
        outside.promiscuity<-(sum(x[j, out.cluster])/length(out.cluster))
        score<-inside.stability-outside.promiscuity
      }
      error.df1<-data.frame(rn=as.character(i),cell=as.character(j),score=score, Promiscuity=outside.promiscuity, Stability=inside.stability, cluster.size=length(in.cluster))
      error.df2<-rbind(error.df, error.df1)
      error.df<-error.df2
    }
  }
...
estimate_noiseparameters<-function(x,y,z, plot=FALSE, sd_inflate=0, normseq=TRUE,granularity=300,write.noise.model=TRUE,file="noise_estimation",model_view=c("Observed","Optimized"),total_sampling=2500, dropout_inflate=1){
  ...
  ERCC.prepared.counts<-prepare_data(x, y, estimateSizeFactors, normseq=normseq)
  if (normseq==TRUE){
    print("Normalizing counts by library size.")
    actual.prepared.counts<-prepare_data_counts(x,z, estimateSizeFactors)
  } else {
    print("Counts will not be normalized as specified by normseq flag.")
...
##needs editing...
plot_obs_spread<-function(x,y, file="Rplot",normseq=FALSE,parameters){
  ERCC.prepared.counts<-prepare_data(x, y, estimateSizeFactors, normseq=normseq)
  ERCC.m.counts<-melt(data.table(ERCC.prepared.counts, keep.rownames = TRUE), id.vars = c("rn", "transcripts"))
  ERCC.m.valid<-ERCC.m.counts[,mean(value)>0, by="rn"]
  setkey(ERCC.m.valid, "rn")
...
report_cell_metrics<-function(y,x){
  ...
  y<-data.table(y)
  cell.scores<-list()
  cell.scores<-lapply(y, `calculate_cell_stability`,x=x, z=z)
  cell.scores.m<-data.table(melt(cell.scores, id.var=c("rn","cell","cluster.size")))
  cell.scores.m$L1<-as.factor(gsub("^X","", cell.scores.m$L1))
  cell.scores.m
}
...
report_cluster_metrics<-function(y,x, plot=FALSE, file="Rplot"){
  z<-rownames(y)
  y<-data.table(y)
  cluster.scores<-list()
  oneclust.stability<-(sum(x)-dim(x)[1])/dim(x)[1]^2
  cluster.scores$X1<-data.frame(rn="1", score=oneclust.stability,Promiscuity=0,Stability=oneclust.stability, size=dim(x)[1])
  cluster.scores<-append(cluster.scores, lapply(y, `calculate_cluster_stability`,x=x, z=z))
  cluster.scores.m<-data.table(melt(cluster.scores, id.var=c("rn","size")))
  ...
  mean.values<-cluster.scores.m[,mean(value), by=c("singlet", "L1", "variable")]
  setkeyv(mean.values, c("singlet", "L1", "variable"))
  colnames(mean.values)[4]<-"Mean.singlets"
  setkeyv(cluster.scores.m, c("singlet", "L1", "variable"))
  cluster.scores.m<-cluster.scores.m[mean.values]
  mean.nosinglet.values<-mean.values[singlet!="Cell number = 1",]
  setkeyv(cluster.scores.m,c("L1", "variable"))
  setkeyv(mean.nosinglet.values, c("L1", "variable"))
  colnames(mean.nosinglet.values)[4]<-"Overall.mean"
  cluster.scores.m<-cluster.scores.m[mean.nosinglet.values[,-1,with=FALSE]]
  ...
  print(level)
  print(cluster.scores.m$L1)
  cluster.scores.m$L1<-factor(cluster.scores.m$L1, levels = level)
  print(cluster.scores.m$L1)
  if (plot==TRUE){
    g<-ggplot(data=cluster.scores.m, aes(x=L1, y=value))+geom_boxplot(aes(alpha=singlet),color="grey20", fill="grey65")+
      guides(fill="none", color="none")+xlab("\nCluster number")+ylab("Cluster metric\n")+facet_wrap(~variable, ncol=1,nrow=3, scales = "free")+
      ggtitle("Cluster technical consensus scores for various clustering\n")+scale_alpha_manual(values=c(1,0.1))+labs(alpha="Cluster size")+
      geom_point(aes(x=factor(L1), y=Mean.singlets, alpha=singlet),fill="red", size=2.5, color="black", pch=23)+annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf)+
      annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf)+theme(axis.text.x = element_text(angle = 90, hjust = 1))
    savefile<-paste(file, "_cluster_singlets_seperated_scores.pdf", sep="")
