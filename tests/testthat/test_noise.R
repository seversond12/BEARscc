...
results<-estimate_noiseparameters(ERCC.counts.df, data.counts.df, 
    ERCC.meta.df, granularity=30, write.noise.model=FALSE, plot=FALSE, 
    alpha_granularity = 0.25)
test_that("estimation output?", {
    expect_true(is.list(results))
    expect_equal(dim(results$bayes_parameters)[2], 
        dim(results$original.counts)[1]+4)
...
test_that("noise perturbation output?", {
    test_noisy_counts<-function(x){
        ...
        expect_equal(dim(noisy_counts.dt)[2], 
            dim(results$original.counts)[2]+1)
